...
import { AlertTriangle, CircleAlert, Info, Pencil } from "lucide-react";
...
export default async function DataQualityPage() {
  ...
  const sourceLimitedMatches = countSourceLimitedMatches(matches);

  return (
    <div className="grid gap-5">
      <div>
        <h1 className="text-2xl font-semibold">Kontrola danych</h1>
        <p className="text-sm text-zinc-500">
          Automatyczne wykrywanie braków i nielogicznych wartości.
        </p>
      </div>
      ...
      {sourceLimitedMatches > 0 ? (
        <div className="flex items-start gap-2 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-800 dark:border-blue-900 dark:bg-blue-950/30 dark:text-blue-200">
          ...
          <div>
            <div className="font-medium">Ograniczenie źródła danych</div>
            <div className="mt-0.5 text-xs opacity-90">
              {sourceLimitedMatches} meczów z Football-Data.co.uk nie ma spalonych.
              Te pola pozostają puste, ale nie są liczone jako problemy jakości danych.
            </div>
          </div>
        </div>
      ) : null}
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <Card className="p-5">
          ...
          <div className="mt-1 text-3xl font-semibold">{issues.length}</div>
        </Card>
        <Card className="p-5">
              ...
              {issues.map((issue) => (
                <tr key={issue.key}>
                  <td className="p-3">
                    ...
                    >
                      {issue.severity === "error" ? (
                        <CircleAlert size={13} className="mr-1" />
                      ) : (
                  ...
                  <td className="p-3">
                    <Link
                      href={`/matches/${issue.matchId}/edit`}
                      className="inline-flex rounded-lg p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800"
                    >
                      <Pencil size={16} />
                    </Link>
                  </td>
