...
function takeRecent<T>(items: T[], lookback: AnalysisLookback) {
  return lookback === null ? items : items.slice(0, lookback);
}
...
export async function loadMatchAnalysis(input: {
  ...
  return measureServerOperation("load-match-analysis", async () => {
    ...
    const [dataset, h2h, refereeMatches, customLines] =
      await Promise.all([
        loadSeasonAnalysisDataset({
    ...
    const recentMatches = [...dataset.matches].reverse();
    const homeRecent = takeRecent(
      recentMatches.filter((item) =>
        includesTeam(item, match.homeTeamId),
      ),
      input.lookback,
    );
    const awayRecent = takeRecent(
      recentMatches.filter((item) =>
        includesTeam(item, match.awayTeamId),
      ),
      input.lookback,
    );
    const homeVenue = takeRecent(
      recentMatches.filter(
        (item) => item.homeTeamId === match.homeTeamId,
      ),
      input.lookback,
    );
    const awayVenue = takeRecent(
      recentMatches.filter(
        (item) => item.awayTeamId === match.awayTeamId,
      ),
      input.lookback,
    );
    ...
    const customLineRows = customLines.flatMap((line) => {
      const definition = TREND_STAT_DEFINITIONS.find(
        (item) => item.key === line.statKey,
      );
...
export type LoadedMatchAnalysis = NonNullable<
  Awaited<ReturnType<typeof loadMatchAnalysis>>
>;
export type LoadedHistoryMatch =
  LoadedMatchAnalysis["homeRecent"][number];
